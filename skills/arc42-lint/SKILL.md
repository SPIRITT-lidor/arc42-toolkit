...
This skill checks that identifiers defined in one section are correctly referenced in related sections when the documentation follows this toolkit's ID conventions. These checks are useful consistency rules, not official arc42 requirements. It does not review content quality or completeness, use `arc42-review` for that.
